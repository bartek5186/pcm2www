...
func cleanEAN(s string) string {
	return integrations.NormalizeEAN(s)
}
...
func (i *Importer) LinkProductsByEAN() error {
	return i.db.Transaction(func(tx *gorm.DB) error {
		...
		sourceByEAN := make(map[string][]int64, len(st))
		for _, p := range st {
			ean := cleanEAN(p.Kod)
			if ean != "" {
				sourceByEAN[ean] = append(sourceByEAN[ean], p.TowarID)
			}
		}
		...
		for _, p := range st {
			...
			if ean == "" {
				...
				if err := saveLinkIssue(tx, p.TowarID, p.Kod, "",
					"missing_ean_src",
					"Brak EAN w eksporcie (pole 'kod' puste/niecyfrowe)"); err != nil {
					return err
				}
				continue
			}
			if sourceIDs := sourceByEAN[ean]; len(sourceIDs) > 1 {
				duplicateSourceEAN++
				idsJSON, _ := json.Marshal(sourceIDs)
				if err := saveLinkIssue(tx, p.TowarID, p.Kod, string(idsJSON),
					"duplicate_ean_source",
					fmt.Sprintf("EAN=%s występuje przy %d produktach PCM (towar_id: %v)", ean, len(sourceIDs), sourceIDs)); err != nil {
					return err
				}
				continue
			}
			...
			case 0:
				...
				if err := saveLinkIssue(tx, p.TowarID, p.Kod, "",
					...
					fmt.Sprintf("Brak produktu o EAN=%s w Woo", ean)); err != nil {
					return err
				}
			...
			default:
				...
				if err := saveLinkIssue(tx, p.TowarID, p.Kod, string(idsJSON),
					"duplicate_ean_shop",
					fmt.Sprintf("EAN=%s występuje %d× w Woo (woo_id: %v)", ean, len(cands), cands)); err != nil {
					return err
				}
			}
		}
		...
		for _, w := range wc {
			...
			if _, exists := magEans[ean]; !exists {
				...
				if err := saveLinkIssue(tx, 0, ean, idsJSON,
					"missing_in_magazine_by_ean",
					fmt.Sprintf("Produkt o EAN=%s jest w Woo (woo_id=%d), ale nie ma go w magazynie", ean, w.WooID)); err != nil {
					return err
				}
			}
		}
		...
		i.log.Info().
			...
			Int("duplicate_ean_source", duplicateSourceEAN).
...
func saveLinkIssue(tx *gorm.DB, towarID int64, kod, wooIDs, reason, details string) error {
	issue := db.LinkIssue{
		TowarID: towarID,
		Kod:     kod,
		WooIDs:  wooIDs,
